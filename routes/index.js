...
const stripe = require("stripe")(
  "sk_test_51Pfv0NA2iC3BEjckGvSQYmoKD20dILThGUrFTowRFbnCTCpP3ehyzu5dVlXp0kRMrr2EfjTmD6FQTYy3UKFvrv4h00xvCgFspv"
);
...
router.post('/addEvents', async (req, res) => {
    try {
        ...
        const newEvent = new ActivityModel(req.body);
        await newEvent.save();
...
router.delete("/events/:id", async (req, res) => {
  try {
    const deletedEvent = await ActivityModel.findByIdAndDelete(req.params.id);
    if (!deletedEvent)
      return res.status(404).json({ message: "Event not found" });
...
router.post(
  "/movies/:id/bookseat/:seating_plan_id/:seat_number",
  async (req, res) => {
    try {
        ...
        for (const theater of movie.showtimes[city]) {
          for (const availability of theater.availability) {
            if (availability.seating_plan.seating_plan_id === seating_plan_id) {
              if (
                availability.seating_plan.booked_seats.includes(seat_number)
              ) {
                return res.status(400).json({ message: "Seat already booked" });
              } else {
                availability.seating_plan.booked_seats.push(
      ...
      res
        .status(200)
        .json({ message: "Seat booked successfully", data: movie });
    } catch (error) {
      res.status(500).json({ message: error.message });
    }
  }
